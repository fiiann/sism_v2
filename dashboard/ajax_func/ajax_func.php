...
if(isset($_GET['search'])){
	...
	if(isset($_GET['pkt'])){
		...
		if($search==''){
			$query="SELECT count(detail_transaksi.idbuku) as jml_data FROM peminjaman
			...
			INNER JOIN buku ON detail_transaksi.idbuku = buku.idbuku WHERE detail_transaksi.tgl_kembali = date(0000-00-00)";
		}else{	
			$query="SELECT count(detail_transaksi.idbuku) as jml_data FROM peminjaman
			INNER JOIN anggota ON peminjaman.nim = anggota.nim
			INNER JOIN detail_transaksi ON detail_transaksi.idtransaksi = peminjaman.idtransaksi 
			INNER JOIN buku ON detail_transaksi.idbuku = buku.idbuku WHERE detail_transaksi.tgl_kembali = date(0000-00-00) AND peminjaman.nim='".$search."'";
		}
	}
