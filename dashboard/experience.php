...
// $query = " SELECT * FROM mekanik m INNER JOIN work_order w ON m.id_mekanik=w.mekanik1  WHERE id_mekanik='".$id_mekanik."'";
	...
	$query = "SELECT *,s.nama_sub,mj.nama_job, jd.nama_detail, COUNT(*) as jumlah FROM mekanik m INNER JOIN work_order w ON m.id_mekanik=w.mekanik1 INNER JOIN sub_section s ON w.id_sub=s.id_sub INNER JOIN main_job mj on mj.id_job=w.id_job INNER JOIN job_detail jd ON jd.id_jobdetail=w.id_detailjob WHERE w.mekanik1='".$id_mekanik."' GROUP BY w.id_detailjob";
